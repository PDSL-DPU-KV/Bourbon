...
Segment GreedyPLR::process(const struct point& pt, bool file) {
  Segment s = {0, 0, 0};
...
Segment GreedyPLR::process__(struct point pt, bool file) {
...
PLR::PLR(double gamma) { this->gamma = gamma; }

std::vector<Segment>& PLR::train(std::vector<string>& keys, bool file) {
  ...
  for (int i = 0; i < size; ++i) {
    ...
    if (!file && ++count % 10 == 0 &&
        adgMod::env->compaction_awaiting.load() != 0) {
      segments.clear();
      return segments;
    }
  }
