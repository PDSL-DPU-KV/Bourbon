...
int main(int argc, char* argv[]) {
  ...
  commandline_options.add_options()(
      ...
      cxxopts::value<int>(num_gets)->default_value("1024"))(
      "l,lower_bound", "the lower bound of the loop of the size of db",
      cxxopts::value<float>(num_pair_lower)->default_value("10"))(
      "u,upper_bound", "the upper bound of the loop of the size of db",
      cxxopts::value<float>(num_pair_upper)->default_value("10"))(
      ...
      "p,profile_out", "the file for profiler output",
      cxxopts::value<string>(profiler_out)->default_value("/tmp/profiler.out"))(
  ...
  for (size_t outer = 0; outer < num_pairs.size(); ++outer) {
    ...
    adgMod::test_num_level_segments =
        (uint32_t)floor(num_pairs[outer] * test_num_segments_base);
    for (size_t iteration = 0; iteration < num_iteration; ++iteration) {
      ...
      for (int cut = cut_size - 1; cut >= 0; --cut) {
        PutAndPrefetch(keys.size() * cut / cut_size,
                       keys.size() * (cut + 1) / cut_size, keys, cut_size);
      }
      ...
      cout << num_pairs[outer] << " " << instance->ReportTime(4) << " "
           << iteration << endl;
      instance->ReportTime();
    ...
    for (int s = 0; s < time_sums.size(); ++s) {
      printf("%d : Time Average for Timer %d : %lu\n", int(num_pairs[outer]), s,
             time_sums[s] / num_iteration);
    }
