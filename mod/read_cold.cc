...
int main(int argc, char* argv[]) {
  ...
  commandline_options.add_options()(
      ...
      "m,modification",
      "if set, run our modified version, 7 fjle-model bourbon, 8 wiskey, 9 "
      "ours",
      ...
      cxxopts::value<int>(adgMod::key_size)->default_value("16"))(
      ...
      "file_model_error", "error in file model",
      cxxopts::value<uint32_t>(adgMod::file_model_error)->default_value("8"))(
      "level_model_error", "error in level model",
      cxxopts::value<uint32_t>(adgMod::level_model_error)->default_value("1"))(
      ...
      "mix", "portion of writes in workload in 1000 operations",
  ...
  for (size_t iteration = 0; iteration < num_iteration; ++iteration) {
    ...
    std::uniform_int_distribution<uint64_t> uniform_dist_file(
    ...
    if (fresh_write && iteration == 0) {
      ...
      //            rc = system("sudo fstrim -a -v");
      //            rc = system("sync; echo 3 | sudo tee
      //            /proc/sys/vm/drop_caches"); cout << "delete and trim
      //            complete" << endl;
      ...
      for (int i = 0; i < keys.size(); ++i) fd << keys[i] << "\n";
      fd.close();
      ...
      if (adgMod::MOD == 6 || adgMod::MOD == 7 || adgMod::MOD == 9) {
        ...
        for (int i = 1; i < config::kNumLevels; ++i) {
          LearnedIndexData::LevelLearn(
              new VersionAndSelf{current, adgMod::db->version_count,
    ...
    // perform workloads according to given distribution, read-write percentage,
    // YCSB workload. (If they are set.)
    ...
    for (int i = 0; i < num_operations; ++i) {
      ...
      if (write) {
        ...
        if (input_filename.empty()) {
          ...
          status = db->Get(read_options,
                           generate_key(to_string(distribution[i])), &value);
          ...
          uint64_t index = use_distribution
                               ? distribution[i]
                               : uniform_dist_file2(e2) % (keys.size() - 1);
          ...
          if (insert_bound != 0 && index > insert_bound) {
            ...
            status =
                db->Get(read_options,
                        generate_key(to_string(10000000000 + index)), &value);
          } else {
            // other
...
//            }
      ...
      if ((i + 1) % (num_operations / 10) == 0) {
        ...
        cout << "Progress:" << (i + 1) / (num_operations / 10) * 10 << "%"
             << endl;
        Version* current = adgMod::db->versions_->current();
        printf("LevelSize %d %d %d %d %d %d\n", current->NumFiles(0),
               current->NumFiles(1), current->NumFiles(2), current->NumFiles(3),
    ...
    for (auto it : file_stats) {
      printf("FileStats %d %d %lu %lu %u %u %lu %d\n", it.first,
             ...
             it.second.num_lookup_pos, it.second.num_lookup_neg, it.second.size,
             it.first < file_data->watermark ? 0 : 1);
    }
  ...
  if (num_iteration > 1) {
    ...
    for (int s = 0; s < times.size(); ++s) {
      ...
      std::transform(time.begin() + 1, time.end(), diff.begin(),
                     [mean](double x) { return x - mean; });
      double stdev = std::sqrt(
          std::inner_product(diff.begin(), diff.end(), diff.begin(), 0.0) /
          time.size());
