...
// the time we wait before learning (as the file may die within this short time
// and if we learn, we waste the learning)
...
string generate_value(uint64_t value) {
  ...
  string result =
      string(value_size - value_string.length(), '0') + value_string;
  return std::move(result);
}
...
uint64_t get_time_difference(timespec start, timespec stop) {
  return (stop.tv_sec - start.tv_sec) * 1000000000 + stop.tv_nsec -
         start.tv_nsec;
}
