...
// During our experiments, we unlimit file descriptor numbers. LevelDB default
// is 1024
...
// constants determined during the first offline learning following the load of
// DB
...
// bool SearchNumEntriesArray(const std::vector<uint64_t>& num_entries_array,
// const uint64_t position, size_t* index, uint64_t* relative_position);
...
class FileStats {
 public:
  ...
  explicit FileStats(int level_, uint64_t size_)
    ...
    uint32_t dummy;
    start = (__rdtscp(&dummy) - instance->initial_time) /
  ...
  void Finish() {
    adgMod::Stats* instance = adgMod::Stats::GetInstance();
    uint32_t dummy;
    end = (__rdtscp(&dummy) - instance->initial_time) /
          adgMod::reference_frequency;
  }
};
}  // namespace adgMod
