...
CBModel_Learn::CBModel_Learn()
    : negative_lookups_time(2), positive_lookups_time(2){};

void CBModel_Learn::AddLookupData(int level, bool positive, bool model,
                                  uint64_t value) {
  leveldb::MutexLock guard(&lookup_mutex);
  std::vector<Counter>& target =
      positive ? positive_lookups_time : negative_lookups_time;
...
double CBModel_Learn::CalculateCB(int level, uint64_t file_size) {
  ...
  uint64_t time_pos[2] = {0, 0}, time_neg[2] = {0, 0}, num_neg_lookups_file,
           num_pos_lookups_file, size_sum, cost_sum, learn_size_sum;
  {
    leveldb::MutexLock guard(&lookup_mutex);
    for (int i = 0; i < 2; ++i) {
  ...
  double pos_gain =
      (average_pos_time[0] - average_pos_time[1]) * average_pos_lookups;
  double neg_gain =
      (average_neg_time[0] - average_neg_time[1]) * average_neg_lookups;
  ...
  //    file_size * const_size_to_cost, level,
  //             average_pos_time[0], average_pos_time[1], average_pos_lookups,
  //             average_neg_time[0], average_neg_time[1], average_neg_lookups);
  return (pos_gain + neg_gain) / size_sum *
         num_files;  //(num_learn >= 20 ? (double) cost_sum / learn_size_sum :
                     // const_size_to_cost);
}
