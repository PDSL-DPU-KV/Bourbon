...
// An array collecting the total number of keys in a level in or before each
// file. One per level. Used to get the target file when a level model produces
// the predicted position in the level.
class AccumulatedNumEntriesArray {
 ...
 public:
  ...
  // During learning, add info to this array with the number of entries in a
  // file and its largest key
...
class LearnedIndexData {
 ...
 public:
  ...
  //        int num_neg_model = 0, num_pos_model = 0, num_neg_baseline = 0,
  //        num_pos_baseline = 0; uint64_t time_neg_model = 0, time_pos_model =
  //        0, time_neg_baseline = 0, time_pos_baseline = 0;
  //
  //        int num_neg_model_p = 0, num_pos_model_p = 0, num_neg_baseline_p =
  //        0, num_pos_baseline_p = 0, num_files_p = 0; uint64_t
  //        time_neg_model_p = 0, time_pos_model_p = 0, time_neg_baseline_p = 0,
  //        time_pos_baseline_p = 0; double gain_p = 0; uint64_t file_size = 0;

  explicit LearnedIndexData(int allowed_seek, bool level_model)
      : error(level_model ? level_model_error : file_model_error),
        learned(false),
        aborted(false),
        learning(false),
        learned_not_atomic(false),
        allowed_seek(allowed_seek),
        current_seek(0),
        filled(false),
        is_level(level_model),
        level(0),
        served(0),
        cost(0){};
  ...
  // If the key is in the training set, the output interval guarantees to
  // include the key otherwise, the output is undefined! If the output lower
  // bound is larger than MaxPosition(), the target key is not in the file
...
// an array storing all file models and provide similar access interface with
